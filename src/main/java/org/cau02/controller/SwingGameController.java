...
public class SwingGameController {
    ...
    private static final int DEFAULT_PIECE_COUNT = 2; // 플레이어당 4개의 말

    public SwingGameController(SwingGame game, SwingGameView view) {
        this.game = game;
        this.view = view;
    }
    ...
    private void setDefaultGameSettings() {
        try {
            // 기본 게임 매개변수 설정
            game.setBoard(DEFAULT_BOARD_SHAPE);
            game.setPlayerCount(DEFAULT_PLAYER_COUNT);
            game.setPieceCount(DEFAULT_PIECE_COUNT);
